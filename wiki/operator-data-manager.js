...
export class OperatorDataManager {
    ...
    async initialize() {
        ...
        try {
            ...
            console.error('[Wiki System] Step 2: Loading search index...');
            const indexStart = Date.now();
            await this.searchIndexer.loadIndex();
            console.error(`[Wiki System] Search index loaded in ${Date.now() - indexStart}ms`);
        ...
        try {
            ...
            if (entriesToIndex.length > 0) {
                ...
                if (searchStats.totalEntries === 0) {
                    console.error(`[Wiki System] Search index is empty, rebuilding from ${entriesToIndex.length} loaded operators...`);
                    const rebuildStart = Date.now();
                    await this.searchIndexer.indexEntries(entriesToIndex, {
                        batchSize: 50,
                        onProgress: (progress) => {
                            if (progress.processed % 100 === 0 || progress.complete) {
                                console.error(`[Wiki System] Indexing progress: ${progress.processed}/${progress.total}`);
                            }
                        }
                    });
                    const rebuildTime = Date.now() - rebuildStart;
                    console.error(`[Wiki System] Search index rebuilt successfully in ${rebuildTime}ms`);
                }
            }
            
        } catch (error) {
            console.error('[Wiki System] No processed entries found, starting fresh');
        }
    }
